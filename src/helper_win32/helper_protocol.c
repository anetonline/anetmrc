...
void helper_log(const char *fmt, ...) {
    ...
    fprintf(g_log, "[N%d] ", g_cur_node ? g_cur_node->node_num : -1);
    va_start(ap, fmt);
...
static void handle_mrc_packet(char *line) {
    ...
    if (strcmp(from_user, "SERVER") == 0) {
        ...
        if (stricmp(to_room, "ctcp_echo_channel") == 0 ||
            ...
            if (strncmp(body, "[CTCP]", 6) == 0) {
                ...
                /* Only display the reply if it's addressed to us. MRC relays
                 * ctcp_echo_channel packets to every listener, so we must
                 * filter by to_user (field 4). */
                if (stricmp(to_user, g_mrc.user) == 0 ||
                    stricmp(to_user, g_mrc.nick) == 0) {
    ...
    if (stricmp(to_room, "ctcp_echo_channel") == 0 ||
        ...
        if (strncmp(body, "[CTCP]", 6) == 0) {
            ...
            /* Only display the reply if it's addressed to us. MRC relays
             * ctcp_echo_channel packets to every listener, so we must
             * filter by to_user (field 4). Without this gate, every user
             * on anetmrc sees the reply to someone else's CTCP query. */
            if (stricmp(to_user, g_mrc.user) == 0 ||
                stricmp(to_user, g_mrc.nick) == 0) {
                bridge_write("|08[|14CTCP-REPLY|08]|07 %s", body + 13);
            }
    ...
    /* ---- Direct messages ---- */
    /* A user-addressed packet (to_user is a handle, not SERVER/CLIENT/NOTME).
     * MRC relays DM packets to every listener, so we must drop any DM not
     * addressed to us — otherwise it falls through to the room-chat branch
     * below and displays as if it were a public message. */
    if (to_user[0] &&
        stricmp(to_user, "SERVER") != 0 &&
        stricmp(to_user, "CLIENT") != 0 &&
        stricmp(to_user, "NOTME")  != 0) {
        if (stricmp(to_user, g_mrc.user) != 0 &&
            stricmp(to_user, g_mrc.nick) != 0) {
            /* Addressed to someone else — swallow silently. */
            return;
        }
...
void helper_run(void) {
    ...
    /* Initialise node slots:
     *   slot 0 → ANETDOS.OUT/IN   (COM1, node 1)
     *   slot 1 → ANETDOS0.OUT/IN  (--local, dev/testing)
     *   slot N → ANETDOS[N].OUT/IN (COM[N])
     */
    for (i = 0; i < MAX_NODES; i++) {
        if (g_node_fixed && i == 0) continue;
        memset(&g_nodes[i], 0, sizeof(g_nodes[i]));
        node_init_defaults(&g_nodes[i]);
        g_nodes[i].node_num = i;
        if (i == 0) {
            ...
            safe_copy(g_nodes[i].bridge_out, sizeof(g_nodes[i].bridge_out), "ANETDOS0.OUT");
            safe_copy(g_nodes[i].bridge_in,  sizeof(g_nodes[i].bridge_in),  "ANETDOS0.IN");
        } else {
